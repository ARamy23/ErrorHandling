...
extension URLSession {
  func data<T: Decodable>(for urlRequest: URLRequest, expects expectedType: T.Type, delegate: URLSessionTaskDelegate? = nil) async throws -> T {
...
class EHJSONDecoder: JSONDecoder {
  ...
  override func decode<T>(_ type: T.Type, from data: Data) throws -> T where T : Decodable {
    do {
      ...
      LoggersManager.error(message: "Error in read(from:ofType:) domain= \(error.domain), description= \(error.localizedDescription)")
      throw error
    } catch {
